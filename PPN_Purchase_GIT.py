...
if not Purchase_PATH:
    raise Exception("Please add the Purchase100 dataset path to the code")



print("Loading dataset...")
data_set =np.genfromtxt(Purchase_PATH,delimiter=',')
...
#         att_inp = att_inp.view([attack_input.size(0),1,2,attack_input.size(1)])
        ...
        losses.update(loss_attack.data[0], attack_input.size(0))
...
def test_attack(train_data,labels,attack_data,attack_label, model,attack_model, criterion,attack_criterion, optimizer,attack_optimizer, epoch, use_cuda):
    ...
    for ind in xrange(len_t):
        ...
        prec1=np.mean(np.equal((attack_output.data.cpu().numpy() >0.5),(v_is_member_labels.data.cpu().numpy()> 0.5)))
        losses.update(loss.data[0], attack_input.size(0))
        top1.update(prec1, attack_input.size(0))
...
for epoch in range(0, epochs):
    ...
    if epoch == 0: 
        ...
        for i in range(76):
            ...
            losses_att.append(at_loss)
            att_accs.append(at_acc)
            att_epoch_acc = np.mean(att_accs)
            tr_loss,tr_acc=train_privatly(train_classifier_data_tensor,train_classifier_label_tensor, model,attack_model, criterion, optimizer, epoch, use_cuda,2,(2*i)%152,find_alpha(att_epoch_acc))
            losses_def.append(tr_loss)
